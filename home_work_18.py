...
user_quantity = input('Введите количество пословиц: ')

user_proverbs = []

if not user_quantity.isdigit():
    print('Вы ввели не число')
elif int(user_quantity) > min(len(proverbs), len(variants)):
    print(f'Количество пословиц превышает доступное число.  Можно сгенерировать максимум {min(len(proverbs), len(variants))} пословиц')
else:
    while len(user_proverbs) < int(user_quantity) and proverbs and variants:
        proverb = random.choice(list(proverbs))
        variant = random.choice(list(variants))
        proverbs.remove(proverb)
        variants.remove(variant)
        user_proverb = proverb.lower().replace('ум',variant.capitalize())
        user_proverbs.append(user_proverb)
    pprint(user_proverbs)
